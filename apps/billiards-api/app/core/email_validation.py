...
def normalize_required_email(value: str, *, allow_test_domains: bool = True) -> str:
    normalized = normalize_optional_email(value, allow_test_domains=allow_test_domains)
    if not normalized:
        raise ValueError("value is not a valid email address")
    return normalized
